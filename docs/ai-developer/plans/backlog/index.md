...
| [PLAN-rancher-124-and-host-checks.md](PLAN-rancher-124-and-host-checks.md) | Rancher Desktop 1.24 + Windows 11/x64 checks on Windows; Mac Rancher scripts to legacy (Jamf installs natively). Approved in principle on #1510, waiting on review | High |
| [PLAN-github-actions-ci.md](PLAN-github-actions-ci.md) | CI on every PR: bash + PowerShell validation, and a .intunewin build + test with **no upload**. The only way to validate PowerShell anywhere in the fleet | **Highest**: gates PR #18 |
